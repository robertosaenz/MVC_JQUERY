IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'EVA_SaeTramiteSolicitud_GenerarOrden') DROP PROCEDURE EVA_SaeTramiteSolicitud_GenerarOrden
GO
--------------------------------------------------------------------------------
...
DECLARE @Output INT = 0
EXEC EVA_SaeTramiteSolicitud_GenerarOrden 'FC', 562001, '00002500', '3070700010', 1, '20999999999',NULL, 'Prueba.SAC','prueba@gmail.com','Prueba 123','01','01','01', @Output OUTPUT
SELECT @Output
*/

CREATE PROCEDURE [EVA_SaeTramiteSolicitud_GenerarOrden]
...
BEGIN
	...
	@Serie = S.Serie,
	@Sucursal = S.Sucursal,
	@EstablecimientoCodigo = S.CodigoEstablecimiento
	FROM EVA_SAE_TramiteSolicitud TS
	INNER JOIN Sede S WITH(NOLOCK) ON S.IdSede = TS.IdSede
	INNER JOIN Matricula M WITH(NOLOCK) ON M.IdMatricula = TS.IdMatricula
	...
	--@Sucursal = S.Sucursal,
	--@EstablecimientoCodigo = CodigoEstablecimiento
	...
	SET @Fecha = getdate()  
	SET @FechaVenc = DATEADD(YEAR,-1, getdate())
	SET @Monedacodigo = 'LO'
	...
	IF @IdSede in (4) --@3 SEDE ICA = 3 Y CHINCHA = 22 Retirada  // condicional de empresa
		BEGIN
			IF @TipoDocumento = 'FC'  
			BEGIN  
